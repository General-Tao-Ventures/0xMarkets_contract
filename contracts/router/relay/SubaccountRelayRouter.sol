...
import "./BaseRelayRouter.sol";

// Orders signed by a session key (subaccount) that the main account approved once, submitted by our
// relayer. The main account signs the approval, which carries an expiry and a maximum action count;
// the session key then signs each order. The session key can only act on orders, and the relay fee it
// spends from the main account is capped by value.
contract SubaccountRelayRouter is BaseRelayRouter {
    ...
    ) BaseRelayRouter(_router, _roleStore, _dataStore, _eventEmitter, _oracle, _orderHandler, _orderVault) {}
        ...
        returns (bytes32)
    {
        ...
        // the session key must not be able to redirect funds away from the main account
    ...
    // signed by the main account, so a compromised session key cannot keep itself alive
